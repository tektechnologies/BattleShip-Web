...
export default class Board extends React.Component{
  render(){
    ...
    let rows = ['a', 'b', 'c', 'd', 'e'];
    let generateClassNames = (shots) =>{ //takes in only the user or opponentShots
    ...
    return(
      <React.Fragment>
        ...
        <div className='userViewofOpponentBoard'>
          {generateClassNames(userShots).map((name,i) =>
            <div key={i} className={name}></div>
          )
          }
        </div>
        <div className='userPlacementBoard'>
          {generateClassNames(opponentShots).map((name, i) =>
            <div key={i} className={name}></div>
          )}
        </div>
      </React.Fragment>
